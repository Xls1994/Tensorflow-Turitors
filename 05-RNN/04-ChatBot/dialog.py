...
class Dialog():
    ...
    def build_vocab(self, data_path, vocab_path):
        import codecs
        with codecs.open(data_path, 'r') as content_file:
            content = content_file.read()
            words = self.tokenizer(content)
            words = list(set(words))

        with codecs.open(vocab_path, 'w') as vocab_file:
            for w in words:
                vocab_file.write(w + '\n')
...
def main(_):
    ...
    if FLAGS.data_path and FLAGS.voc_test:
        ...
        print "生成词汇表存放各个单词：", FLAGS.data_path
